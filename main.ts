...
const groupBy = (items: Array<Object>, key: string) =>
	items.reduce(
		(result, item) => ({
			...result,
...
async function tryUntilSucceed<T>(func: () => Promise<T>, delay = 100) {
...
async function getTokensAmount(
	...
	for (const address of wallets) {
		...
		const contracts = new Map<string, [string, number]>();
		...
		for (let contractData of contracts.entries()) {
			...
			results.set(
				contractData[0],
				results.get(contractData[0]) + total * Math.pow(10, -contractData[1][1])
			);
			bar.increment();
		}
		bar.stop();
...
function main() {
	...
	parser.addArgument("--addressedFileName", {
		...
		help:
			"File name containing the wallets to analyze. Addresses must be separated by a new line.",
	});
	...
	parser.addArgument("--sortBy", {
		...
		help:
			"Whether to sort the result according to the symbol (alphabetically) or the amount (ascending).",
	});
